...
ENV_WHITELIST := PATH HOME LANG MAKE% MXE% %PROXY %proxy LD_LIBRARY_PATH $(PRELOAD_VARS) ACLOCAL_PATH PKG_CONFIG_PATH NIX_CONF_DIR NIX_PROFILES NIX_CC NIX_CFLAGS_COMPILE NIX_REMOTE NIX_STORE NIX_LDFLAGS NIX_BUILD_TOP NIX_PATH NIX_OTHER_STORES IN_NIX_SHELL NIXPKGS_CONFIG NIX_BUILD_CORES NIX_USER_PROFILE_DIR LIBVA_DRIVERS_PATH SHELL PERL5LIB CMAKE_LIBRARY_PATH builder NIX_CFLAGS_COMPILE nativeBuildInputs stdenv SLIM_THEMESDIR GDK_PIXBUF_MODULE_FILE shell SSH_ASKPASS out CONFIG_SHELL GTK_PATH PYTHONPATH GEM_PATH  GTK_DATA_PREFIX ACLOCAL_PATH CMAKE_PREFIX_PATH _PATH GIO_EXTTRA_MODULES _
